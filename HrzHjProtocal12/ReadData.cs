            ...
            try
            {
                ...
                if ((Convert.ToUInt16(obj.F118) & 0x01) == 0x01)
                {
                    ...
                    if ((Convert.ToUInt16(obj.F118) & 0x01) == 0x01)
                    {
                        ...
                        {
                            cmd = CreateModbusRtuWriteCommand(recdPtr.m_userid, 0x01, 0x62, Convert.ToUInt16((int)obj.F118 & 0xFFF0));
                ...
                if ((Convert.ToUInt16(obj.F119) & 0x01) == 0x01)
                {
                    //需要把到云存储（供热站级别）中的配置数据，下发到该换热站
                    if ((Convert.ToUInt16(obj.F119) & 0x02) == 0x02)
                    {
                        #region 获取云端配置 生成命令，
                    ...
                    if (cmds != null && cmds.Count > 0)
                    {
                        cmd = CreateModbusRtuWriteCommand(recdPtr.m_userid, 0x01, 0x63, Convert.ToUInt16((int)obj.F118 & 0xFFF0));
                        cmd.NeedSendTime = DateTime.Now;
                        cmd.bsO_id = obj.bsO_Id;
                        cmd.bsP_Id = (int)Dev.bsP_Id;
