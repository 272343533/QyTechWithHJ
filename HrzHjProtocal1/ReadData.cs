    ...
    public class ReadData : IProtocal
    {
        ...
        public override DeviceCmd CreateReadCommand(string simno)
        {
            if (ReadPacketCommand != null && ReadPacketCommand.CommNo != null && !ReadPacketCommand.CommNo.Equals(""))
            {
                ...
                return base.CreateModbusRtuReadCommand(simno, 0x01, 0x03, 0x0, 0x75 - 0x0 + 1);
            //01030000006585E1
        }
            ...
            try
            {
                ...
                foreach (bsProtItem pi in bsProtItems)
                {
                    try
                    {
                        ...
                        obj = (HJGathData1)SetValueByReflectionFromBytes<HJGathData1>(obj, pi, propertyInfo, buff);
                    }
                    catch (Exception ex)
                    {
                ...
                EntityManager<HJGathData1>.Add<HJGathData1>(obj);

                
                NewHJGathData1 newobj = EntityManager<NewHJGathData1>.GetBySql<NewHJGathData1>("Det_Id='" + obj.Det_Id.ToString() + "'");
                if (newobj == null)
                {
                    newobj = EntityOperate.Copy<NewHJGathData1>(obj);
                    EntityManager<NewHJGathData1>.Add<NewHJGathData1>(newobj);
                }
                else
                {
                    //newobj = EntityOperate.Copy<NewHJGathData1>(obj);
                    EntityOperate.Copy<NewHJGathData1>(obj, newobj, "Det_Id");
                    EntityManager<NewHJGathData1>.Modify<NewHJGathData1>(newobj);
                }
