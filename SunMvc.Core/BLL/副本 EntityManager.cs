...
namespace SunMvcExpress.Core.BLL
{
    ...
    {
        public static ILog log = log4net.LogManager.GetLogger("EntityManager");
        ...
        public static List<T> GetAllByStorProcedure(string spName, string paramvalues)
        {
            try
            {
                using (var db = SingletonLinkDb.GetSingle(DbEntities.CommonDb)
                {
        ...
        /// <returns></returns>
        public static string AddWithDbContext<T>(T t, WSExpressEntities db)
        {
            try
            {
                entityName = db.GetType().GetProperties().Where(p => p.Name == t.GetType().Name || p.Name == t.GetType().Name + "s" || p.Name == t.GetType().Name + "es").Single().Name;
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public static string Modify<T>(T t, WSExpressEntities db) where T : EntityObject
        {
            try
            {
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public static string Delete<T>(T t, WSExpressEntities db)
        {
            try
            {
                db.DeleteObject(t);
        ...
        public static List<T> GetListNoPaging<T>(Guid userID, string fkcondtion, string filter, string conditions, string orderbys) where T : class,new()
        {
            try
            {
                ...
                List<T> ret = GetListNoPaging<T>(sql, orderbys);
                return ret;
            }
            catch (Exception ex)
            {
        ...
        public static string ReturnErrMessage(Exception ex)
        {
            string errmsg = entityName + ":" + ex.Message;
            if (ex.InnerException != null)
