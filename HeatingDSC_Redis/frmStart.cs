...
namespace HeatingDSC
{
    ...
    public partial class frmStart : Form//,ISendMessageServiceCallback
    {
                    ...
                    if (slDeviceCmdsUnNoraml[simno].Count > 0)
                    {
                        ...
                        for (int i = 0; i < slDeviceCmdsUnNoraml[simno].Count; i++)
                        {
                            ...
                            if (cmd.Response.Substring(0,6) == packet.Substring(0, 6))
                            {
                                ...
                                if (hds != null)
                                {
                                    //if (hds.ValidResponse.Substring(0, 6) == packet.Substring(0, 6))//都只处理前六位,前面已判断
                                    //{
                                ...
                                #region  判断redis忠是否存在 add on 2019-11-19

                                string hashid = "d:" + cmd.CommNo;
                                string key = cmd.SetDownTime.ToString("MMddHHmmssfff") + "_" + cmd.Command.Substring(4, 4);
                                //string value = "" + DateTime.Now.ToString() + "," + cmd.Command + "," + cmd.Response;
                                //if (qyRedisCacheHelper.Exists(hashid, key))
                                qyRedisCacheHelper.RemoveHashItem(hashid, key);
                    ...
                    try
                    {
                        ...
                        if (cmd.CommNo != "")
                        {
                            ...
                            if (!HaveSameCmd)
                            {
                                ...
                                #region redis add
                                //HashSet< vals = qyRedisCacheHelper.Get<HashSet>("");
                                string hashid = "d:" + cmd.CommNo;
                                string key = cmd.SetDownTime.ToString("MMddHHmmssfff") + "_" + cmd.Command.Substring(4, 4);
                                string value = "" + DateTime.Now.ToString() + "," + cmd.Command + "," + cmd.Response;
                                if (!qyRedisCacheHelper.Exists(hashid, key))
                                    qyRedisCacheHelper.Add2Hash(hashid, key, value);
        ...
        {
            qyRedisCacheHelper.Add2Hash("hashid", "key", "value");
            qyRedisCacheHelper.Add2Hash("hashid", "key1", "value1");
            qyRedisCacheHelper.RemoveHashItem("hashid2", "key2");
            qyRedisCacheHelper.RemoveHashItem("hashid", "key1");


            string a = qyRedisCacheHelper.Get<string>("d:13302127187:0043");
          
            string h = qyRedisCacheHelper.GetHash("d:13300000001", "1911181704223:0043");

            List<string> l = qyRedisCacheHelper.GetList("d:13302127187");
