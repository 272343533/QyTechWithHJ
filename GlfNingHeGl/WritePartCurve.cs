...
namespace QyTech.GlfNingHeGl
{
    ...
    public class WritePartCurve : IProtocal
    {
            ...
            try
            {
                ...
                foreach (bsProtItem pi in bsProtItems)
                {
                    try
                    {
                        ...
                        CrossHiLow(ref buff);
                        if (ItemAddr >= 0x00C1 && ItemAddr <= 0x00D8)
                        {
                            //此处需要除以100才可以
                            object val = IBytesConverter.FromBytes(buff, pi.UnifType);
                            val = Convert.ToInt16(val) / 100;
                            obj = (HjPlcParaControlCurve)QyTech.Core.Refection.BaseFunc.SetValue<HjPlcParaControlCurve>(obj, propertyInfo, val);
                        }
                        else
                        {
        ...
        {
            if (address >= 0x00C1 && address<=0x00D8)
            {
                for(int i=0;i<args.Length;i++)
                {
                    if((address-0x00c1) %2==0)
                        args[i] =Convert.ToInt16(Convert.ToInt16(args[i] ) * 100);
                    else
                        args[i] = Convert.ToUInt16(Convert.ToUInt16(args[i]) * 100);
