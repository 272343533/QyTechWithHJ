...
namespace QyTech.ProtocalGlfSheXian
{
    ...
    /// </summary>
    public class WritePartCurve2 : IProtocal
    {
        public DownHrzCurve downHrzcurve_;

    

        public WritePartCurve2(int bsP_Id)
            : base(bsP_Id)
        {
        }
        ...
        public override DeviceCmd CreateModbusRtuWriteCommandByConf(string simno, object obj)
        {
            ...
            int hwd;
            string val;
            for (int i = 1; i <= (wds.Length<=48?wds.Length:48); i += 1)
            {
                hwd = (int)(Convert.ToUInt16(wds[i - 1]) * 10);
            ...
            {
                hwd = (int)(Convert.ToUInt16(wds[wds.Length - 1]) * 10);
                propertyInfo = type.GetProperty("Temp" + (i * 2 - 1).ToString()); //获取指定名称的属性
                propertyInfo.SetValue(objtarget, hwd, null); //给对应属性赋值
        ...
        //{
        //    return base.CreateModbusRtuReadCommand(simno, 0x01, 0x03, 0x02C4, 0x032F - 0x02C4 + 1);
