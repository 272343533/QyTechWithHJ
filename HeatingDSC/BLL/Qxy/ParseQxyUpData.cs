...
{
    public class ParseQxyUpData:IProtocal
    {
        public static ILog log = log4net.LogManager.GetLogger("ParseQxyUpData");

        //采集数据
        public WeaDataHistory otobj = new WeaDataHistory();
        
      
        public ParseQxyUpData()
        {
        ...
        public override int Parse(GPRS_DATA_RECORD recdPtr)
        {
            ...
            try
            {
                ...
                buff = new byte[2];
                Buffer.BlockCopy(GetData, 0x2, buff, 0, 2); CrossHiLow(ref buff);
                otobj.WDHis = (decimal)(BitConverter.ToInt16(buff, 0) / 10.0);
                Buffer.BlockCopy(GetData, 0x4, buff, 0, 2); CrossHiLow(ref buff);
                otobj.FSHis = (decimal)(BitConverter.ToUInt16(buff, 0) / 10.0);
                Buffer.BlockCopy(GetData, 10, buff, 0, 2); CrossHiLow(ref buff);
        ...
        public List<DeviceCmd> CreateWdzProtQxy(List<DTUProduct> gprsDevs)
        {
            ...
            foreach (DTUProduct d in gprsDevs)
            {
                try
                {
                    ...
                    ModbusCommand.RegOpNum =0x07;//, 16);// Convert.ToInt32(sets[2], 16);//字节数量10进制
