    ...
    public class ParseWqQxyUpData : IProtocal
    {
        ...
        public override int Parse(GPRS_DATA_RECORD recdPtr)
        {
            ...
            try
            {
                ...
                buff = new byte[2];
                Buffer.BlockCopy(GetData, 0x0, buff, 0, 2); CrossHiLow(ref buff);
                otobj.WDHis = (decimal)(BitConverter.ToInt16(buff, 0) / 10.0);

                Buffer.BlockCopy(GetData, 0x0E*2, buff, 0, 2); CrossHiLow(ref buff);
                otobj.RZHis = BitConverter.ToUInt16(buff, 0);

                Buffer.BlockCopy(GetData, 0x16*2, buff, 0, 2); CrossHiLow(ref buff);
        ...
        public List<DeviceCmd> CreateProtQxy(List<DTUProduct> gprsDevs)
        {
            ...
            foreach (DTUProduct d in gprsDevs)
            {
                try
                {
                    ...
                    ModbusCommand.RegOpNum =0x002B;//, 16);// Convert.ToInt32(sets[2], 16);//字节数量10进制
