...
{
    public class Alarm2 : IProtocal
    {
     
        public Alarm2(int bsP_Id):base(bsP_Id)
        {
        }
        ...
        public override int Parse(GPRS_DATA_RECORD recdPtr)
        {
            ...
            try
            {
                ...
                try
                {
                    NewStandDataHrz gathobj = EntityManager<NewStandDataHrz>.GetByPk<NewStandDataHrz>("Det_Id", obj.Det_Id);
        ...
        public override DeviceCmd CreateModbusRtuWriteCommandBySelfDefine(string simno)
        {
            ...
            //Org = BaseDABll.GetOrganize((Guid)Dev.AZWZ);
            //HrzGathAlarm obj = EntityManager<HrzGathAlarm>.GetByPk<HrzGathAlarm>("bsO_Id", Guid.Parse(Org.BllOrgID));

            //return base.CreateModbusRtuWriteCommand<HrzGathAlarm>(simno, obj, bsProtItems);
            //// return new DeviceCmd();


            List<HrzGathAlarm> objs = EntityManager<HrzGathAlarm>.GetAllByStorProcedure<HrzGathAlarm>("splyGetClouldStorageData '", Dev.AZWZ.ToString() + "',2");
            return base.CreateModbusRtuWriteCommand<HrzGathAlarm>(simno, objs[0], bsProtItems);
        }
