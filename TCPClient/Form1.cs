    ...
    public partial class Form1 : Form
    {
        ...
        void _client_MessageReceived(ZProxySocket proxySocket, ZMessage message)
        {
            this.Invoke((Action)(delegate()
            {
               ...
               // if (msg.MsgType == 1)  //文本
                {
                    textBox1.AppendText(DateTime.Now.ToString("MM-dd HH:mm:ss")+ proxySocket.RemoteIP + ":" + proxySocket.RemotePort + " 发送数据：\r\n");
                    textBox1.AppendText("           "+msg.PackContent.StrRawData + "\r\n");
                }
        ...
        {
            string smess = this.textBox2.Text.Replace(" ", "").Replace(",", ".");

            ZdFT_Packet sendpack = new Read0C4003();
            //sendpack.Create(smess);
            sendpack.RawData = strToToHexByte(smess);
            _proxy.SendMessage(new BaseMessage(2,sendpack));  //发送BaseMessage消息
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string address = this.textBox4.Text.Replace(" ", "").Replace(",", ".");
