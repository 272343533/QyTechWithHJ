...
namespace DSC_FTHj1
{
    ...
    public class Write130809 : ZdFT_Packet
    {
        /// <summary>
        /// 设置调试模式设定温度
        /// </summary>
        ...
        public override byte[] Create(string data)
        {
            ...
            dataarr[1] = (Convert.ToDecimal(dataarr[1]) * 100).ToString();
            dataarr[2] = (Convert.ToDecimal(dataarr[2]) * 100).ToString();

            RawData = new byte[20 + 18];
            int ipos = 0;
            RawData[0] = 0x68;
            // 长度
            RawData[1] = (byte)(12 + 18 / 2);// 0x3C;
            RawData[2] = 0x00;
            RawData[3] = RawData[1];
            RawData[4] = RawData[2];
