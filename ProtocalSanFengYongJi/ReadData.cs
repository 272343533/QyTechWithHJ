    ...
    public class ReadData : IProtocal
    {
        ...
        public override DeviceCmd CreateReadCommand(string simno)
        {
            if (ReadPacketCommand != null && ReadPacketCommand.CommNo != null && !ReadPacketCommand.CommNo.Equals(""))
            {
                ...
                return base.CreateModbusRtuReadCommand(simno, 0x01, 0x03, 0x0, 0x74 - 0x0 + 1);
            //01030000006585E1
        }
        ...
        public override int Parse(GPRS_DATA_RECORD recdPtr)
        {
            ...
            try
            {
                ...
                if (true)//(Convert.ToUInt16(obj.F115) & 0x01) == 0x01)
                {
                    ...
                    if ((Convert.ToUInt16(obj.F115) & 0x02) == 0x02)
                    {
                        ...
                        cmd.CommandDesp = "控制参数1数据云采集";
                        cmd.SetDownTime = DateTime.Now;
                        cmds.Add(cmd);
                        ...
                        {
                            cmd = CreateModbusRtuWriteCommand(recdPtr.m_userid, 0x01, 0x62, Convert.ToUInt16((int)obj.F115&0xFFF0));
                            cmd.NeedSendTime = DateTime.Now;
                            cmd.bsO_id = (Guid)obj.bsO_Id;
                            cmd.bsP_Id = (int)Dev.bsP_Id;
                            cmd.CommandDesp = "读云数据恢复写入";
                            cmd.SetDownTime = DateTime.Now;
                            cmds.Add(cmd);
                ...
                if ((Convert.ToUInt16(obj.F116) & 0x01) == 0x01)
                {
                    //需要把到云存储（供热站级别）中的配置数据，下发到该换热站
                    if ((Convert.ToUInt16(obj.F116) & 0x02) == 0x02)
                    {
                        #region 获取云端配置 生成命令，
                        ...
                        cmd.SetDownTime = DateTime.Now;
                        cmd.NeedSendTime = cmd.SetDownTime.AddMinutes(1);
                        cmd.bsO_id = (Guid)obj.bsO_Id;
                        cmd.bsP_Id = (int)Dev.bsP_Id;
                        cmds.Add(cmd);
                    ...
                    if (cmds != null && cmds.Count > 0)
                    {
                        cmd = CreateModbusRtuWriteCommand(recdPtr.m_userid, 0x01, 0x63, Convert.ToUInt16((int)obj.F115&0xFFF0));
                        cmd.NeedSendTime = DateTime.Now;
                        cmd.bsO_id = (Guid)obj.bsO_Id;
                        cmd.bsP_Id = (int)Dev.bsP_Id;
