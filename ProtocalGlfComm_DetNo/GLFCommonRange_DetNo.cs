...
{
    public class GLFCommonRange_DetNo : IProtocal
    {
        protected int DetailDevIndex = -1;
        protected int DetailDevNo = 0;

        public GLFCommonRange_DetNo(int bsP_Id):base(bsP_Id)
        {
        }
        ...
        public override int Parse(GPRS_DATA_RECORD recdPtr)
        {
            ...
            try
            {
                ...
                foreach (bsProtItem pi in bsProtItems)
                {
                    ...
                    {
                        log.Error("Range" + DetailDevNo.ToString() + ".Parse(" + pi.FieldName + "):" + ex.Message);
                    }
                }
