...
{
    public class GLFCommonCoil_DetNo : IProtocal
    {
        ...
        public GLFCommonCoil_DetNo(int bsP_Id):base(bsP_Id)
        {
            ...
            try
            {
                ...
                foreach (bsProtItem pi in bsProtItems)
                {
                    try
                    {
                        ...
                        ByteNo = (ItemAddr - InitAddr)/ 8;
                        bitno = (ItemAddr - InitAddr) % 8;//一般按照整数配置，1个寄存器

                        Buffer.BlockCopy(GetData, ByteNo, buff, 0, bufflen);
                        System.Reflection.PropertyInfo propertyInfo = type.GetProperty(pi.FieldName); //获取指定名称的属性
                        propertyInfo.SetValue(obj, ((int)buff[0]&(int)Math.Pow(2,bitno))== (int)Math.Pow(2, bitno) ? true:false, null);
                        //obj = (OTCoilData)SetValueByReflectionFromBytes<OTCoilData>(obj, pi, propertyInfo, buff);
                    }
                    catch (Exception ex)
                    {
                ...
                EntityManager<OTCoilData>.Add<OTCoilData>(obj);


                NewCoilData newobj = EntityManager<NewCoilData>.GetBySql<NewCoilData>("Det_Id='" + obj.Det_Id.ToString() + "'");
               
                if (newobj == null)
                {
                    newobj = EntityOperate.Copy<NewCoilData>(obj);
                    EntityManager<NewCoilData>.Add<NewCoilData>(newobj);
                }
                else
                {
                    EntityOperate.Copy<NewCoilData>(obj, newobj, "Det_Id");
                    EntityManager<NewCoilData>.Modify<NewCoilData>(newobj);
                }
