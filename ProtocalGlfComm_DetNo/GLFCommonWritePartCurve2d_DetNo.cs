...
namespace QyTech.ProtocalGlfCommon_DetNo
{
    ...
    /// </summary>
    public class GLFCommonWritePartCurve2d_DetNo : IProtocal
    {
        public DownHrzCurve downHrzcurve_;

    

        public GLFCommonWritePartCurve2d_DetNo(int bsP_Id)
            : base(bsP_Id)
        {
        }
        ...
        public override DeviceCmd CreateModbusRtuWriteCommandByConf(string simno, object obj)
        {
            ...
            {
                hwd = Convert.ToUInt16(wds[i - 1]) * 100;
            ...
            {
                hwd =Convert.ToUInt16(wds[wds.Length - 1])*100;
                propertyInfo = type.GetProperty("Temp" + (i * 2 - 1).ToString()); //获取指定名称的属性
                propertyInfo.SetValue(objtarget, hwd, null); //给对应属性赋值
